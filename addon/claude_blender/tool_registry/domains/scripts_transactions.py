...
SPECS = tuple(ToolSpec(**payload) for payload in [{'name': 'draft_script',
 ...
 {'name': 'draft_privileged_script',
  ...
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
 {'name': 'start_trusted_script_job',
  'description': 'Start long generated Blender Python in a background Blender process under active runtime script '
                 'trust. The job edits a copied .blend and returns immediately with a job id; it cannot replace the '
                 'live scene until apply_trusted_script_job_result receives explicit user confirmation.',
  'input_schema': {'type': 'object',
                   'properties': {'intent': {'type': 'string'},
                                  ...
                                  'code': {'type': 'string',
                                           'maxLength': 500000,
                                           'description': 'Complete Blender Python. It may call '
                                                          'agent_job_report_progress(progress, message, details) and '
                                                          'agent_job_cancel_requested().'}},
                   'required': ['intent', 'expected_changes', 'risk_level', 'code'],
                   'additionalProperties': False},
  'contract': {'description': 'Start timeout-safe trusted Python against a copied .blend in background Blender',
               'mutates_scene': False,
               'has_side_effects': True,
               'requires_approval': False,
               'risk_level': 'preview',
               ...
               'open_world_hint': True,
               'timeout_seconds': 60,
               'duration_hint': 'Returns after starting a background Blender process; poll job status.',
               'timeout_recovery': {'recoverable': True,
                                    'poll_after_seconds': 2,
                                    'status_tool': 'get_trusted_script_job_status',
                                    'message': 'Poll the returned trusted script job id before retrying.'}},
  'handler_key': 'start_trusted_script_job',
  'order': 1822,
  'groups': ('advanced_workflow', 'model_quality'),
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
 {'name': 'get_trusted_script_job_status',
  'description': 'Poll a background trusted-script job for progress, status, logs, and result .blend availability.',
  'input_schema': {'type': 'object',
                   'properties': {'job_id': {'type': 'string'}},
                   'required': ['job_id'],
                   'additionalProperties': False},
  'contract': {'description': 'Poll one background trusted-script job',
               'mutates_scene': False,
               'supports_headless': True,
               'permissions': ['scene:read', 'filesystem:read']},
  'handler_key': 'get_trusted_script_job_status',
  'order': 1824,
  'groups': ('advanced_workflow', 'model_quality'),
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
 {'name': 'cancel_trusted_script_job',
  'description': 'Cancel a running background trusted-script job and preserve the unchanged live Blender scene.',
  'input_schema': {'type': 'object',
                   'properties': {'job_id': {'type': 'string'}},
                   'required': ['job_id'],
                   'additionalProperties': False},
  'contract': {'description': 'Cancel one background trusted-script process',
               'mutates_scene': False,
               'has_side_effects': True,
               'risk_level': 'preview',
               'permissions': ['process:terminate', 'filesystem:write']},
  'handler_key': 'cancel_trusted_script_job',
  'order': 1826,
  'groups': ('advanced_workflow', 'model_quality'),
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
 {'name': 'apply_trusted_script_job_result',
  'description': 'Checkpoint the current live file, then open a completed trusted-script job result. Requires an '
                 'explicit true confirmation that the user approved replacing the live Blender file. Active runtime '
                 'script trust is preserved across the file change.',
  'input_schema': {'type': 'object',
                   'properties': {'job_id': {'type': 'string'},
                                  'confirm_replace_current_scene': {
                                      'type': 'boolean',
                                      'description': 'Must be true only after explicit user approval.'}},
                   'required': ['job_id', 'confirm_replace_current_scene'],
                   'additionalProperties': False},
  'contract': {'description': 'Checkpoint and replace the live file with a completed trusted-script job result',
               'mutates_scene': True,
               'has_side_effects': True,
               'requires_approval': True,
               'permissions': ['scene:mutate', 'project:open', 'filesystem:write'],
               'destructive_hint': True,
               'open_world_hint': False,
               'timeout_seconds': 120},
  'handler_key': 'apply_trusted_script_job_result',
  'order': 1828,
  'groups': ('advanced_workflow', 'model_quality'),
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
 {'name': 'list_trusted_script_jobs',
  'description': 'List recent background trusted-script jobs without returning stored generated Python.',
  'input_schema': {'type': 'object',
                   'properties': {'limit': {'type': 'integer', 'minimum': 1, 'maximum': 100}},
                   'additionalProperties': False},
  'contract': {'description': 'List recent background trusted-script jobs',
               'mutates_scene': False,
               'supports_headless': True,
               'permissions': ['filesystem:read']},
  'handler_key': 'list_trusted_script_jobs',
  'order': 1829,
  'groups': ('advanced_workflow', 'model_quality'),
  'exposure': 'catalog',
  'owner': 'scripts_transactions'},
