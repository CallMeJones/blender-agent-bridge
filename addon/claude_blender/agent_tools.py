...
AGENT_GUIDANCE = (
    ...
    "For simulation setup, prefer add_cloth_simulation_to_selected or add_particle_system_to_selected for bounded setup, then inspect with get_simulation_details or inspect_simulation_bake. Persistent simulation/cache bake and cache-freeing scripts are disabled; inspect first, then ask the user to perform the persistent bake manually in Blender. Do not hand the user a checkpoint or recovery .blend path unless you just verified that it exists and is restorable through checkpoint metadata, diagnostics, or a filesystem check. "
    ...
    "Use draft_script for custom or larger advanced scene scripts only when runtime script trust is active and static checks pass; helper overlap should be treated as advice. Privileged generated scripts, including custom filesystem/network/project-file scripts and persistent simulation/cache bakes, are disabled. Use bounded external-asset, project-directory file, render, capture, save, and project-output tools instead. "
    ...
    "Generated Python runs only while the user has enabled runtime script trust; otherwise draft_script is refused without creating a pending approval. "
