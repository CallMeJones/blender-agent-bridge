"""Version-aware Blender docs lookup with a small local cache.
...
def _parse_html_page(filepath, relative_path, version):
    ...
    keywords = sorted(set(_terms(f"{title} {symbol} {' '.join(parser.headings[:5])}")))
    return {
        "id": f"full:{relative_path.replace(os.sep, '/')}",
        "title": title,
        "symbol": symbol,
        "keywords": keywords,
        "url": f"{docs_base_url(version)}{relative_path.replace(os.sep, '/')}",
        "local_path": relative_path.replace(os.sep, "/"),
        "snippet": text[:MAX_SNIPPET_CHARS],
        "source": "full_docs_index",
    }
...
def _index_html_docs(cache_dir, version, max_pages=DEFAULT_MAX_INDEX_PAGES):
    html_root = _find_html_root(_html_dir(cache_dir, version))
    entries = []
    for root, _, files in os.walk(html_root):
        for filename in sorted(files):
            ...
            entry = _parse_html_page(filepath, relative_path, version)
            if entry:
                entries.append(entry)
            if len(entries) >= int(max_pages):
                return entries
    return entries


def build_full_docs_cache(*, cache_dir=None, version=None, force=False, max_pages=DEFAULT_MAX_INDEX_PAGES):
    version = version or blender_docs_version()
    ...
    seed_docs_cache(cache_dir=cache_dir, version=version)
    url = docs_zip_url(version)
    os.makedirs(os.path.dirname(zip_path), exist_ok=True)
    _download_zip(url, zip_path)
    _safe_extract_zip(zip_path, html_path)
    entries = _index_html_docs(cache_dir, version, max_pages=max_pages)
...
def _load_full_entries(cache_dir, version):
    ...
    try:
        ...
        return index_path, []
    return index_path, payload.get("entries", [])
...
def docs_cache_status(*, cache_dir=None, version=None):
    ...
    if os.path.exists(full_index_path):
        try:
            with open(full_index_path, "r", encoding="utf-8") as handle:
                full_entries = int((json.load(handle) or {}).get("entry_count", 0))
...
def search_blender_docs(query, *, cache_dir=None, local_first=True, max_results=5):
    ...
    all_entries = entries + full_entries
