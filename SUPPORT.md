...
- A supported local MCP-capable client from the [client guide matrix](docs/clients/README.md).
...
Use [GitHub Discussions](https://github.com/CallMeJones/blender-agent-bridge/discussions) for help, ideas, and show-and-tell once Discussions are enabled. Use a GitHub bug report for reproducible defects and a feature request for proposals. Include Blender version, operating system, bridge/runtime version, bundled or `uvx` mode, MCP client, reproduction steps, expected behavior, actual behavior, and redacted logs.

There is no official Discord at launch. Synchronous community support will be reconsidered only if recurring demand is demonstrated by at least ten distinct support or contributor conversations in a month.
