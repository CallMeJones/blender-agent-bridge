...
- `create_image_texture_material`: creates or updates a Principled BSDF material from exact local image/PBR map paths, packed ARM/ORM maps, AO, bump maps, and optional UV-map selection, and optionally assigns it to selected or named mesh objects.
- `inspect_material_setup`: read-only material quality gate for image file existence, PBR map color spaces, shader graph links, and UV map assignments.
- `repair_material_setup`: preview-safe material repair for image color spaces and image texture UV-vector links.
- `bake_maps`: bakes bounded AO, normal, and base-color/diffuse maps from selected or named mesh materials to PNG artifacts.
- `create_procedural_texture_material`: creates bounded procedural noise/voronoi/wave/checker texture materials with optional bump and preview rollback.
- `uv_unwrap`: creates or updates preview-safe UV coordinate maps on mesh objects with mesh-data rollback, bounded packing, and layout stats.
- `mark_uv_seams`: marks or clears bounded UV seams from boundary/face-angle rules with mesh-data rollback.
- `inspect_uv_layout`: read-only UV quality gate for missing UVs, bounds, area, seam counts, possible overlap bounds, and scale warnings; detected overlap bounds fail the inspection unless overlap scanning is disabled with `max_overlap_pairs=0`.
...
- `set_render_settings`: sets render engine, resolution, FPS, frame range, transparency, look-dev quality presets, samples, denoise, and color management.
- `set_render_engine`: focused render-engine/look-dev helper for engine, preset, samples, denoise, and color-management changes.
- `configure_render_outputs`: enables/disables ViewLayer render passes and configures shader AOVs with preview rollback.
...
- `create_lookdev_turntable_review`: creates a bounded look-dev stage/turntable, applies render controls, captures inspection stills, and validates image artifacts.
...
Advanced helper coverage is implemented for Principled shader materials, image/PBR and procedural textures, material inspection/repair, UV and map-baking workflows, reusable Geometry Nodes starters, bounded modeling operations (`edit_mesh`, `inspect_modeling_quality`, `curve_to_mesh`, Booleans, mirror/symmetry, solidify, screw, and procedural arrays), shape keys, text, curves, particles, basic armatures, constraints, render/look-dev controls, passes/AOVs, cameras, staging, and visual evidence. The bridge intentionally omits style-specific finished-content generators; open-ended props, products, characters, storyboards, cutouts, and designed shots use composable helpers, asset import, or one session-trusted script.
Reusable refinement helpers remain available for shade smoothing, bevel/subdivision stacks, wheel assemblies, panel seams, window/glass panels, staging, callouts, lighting, palettes, turntables, and scene organization.
...
Status: Safe editing helpers cover the original simple edits plus bounded modeling, material, staging, scene-organization, object visibility/display, and animation operations. Rollback manifests and domain-aware preview reports summarize expected changes and restoration coverage. The former vehicle/product/character refinement kits and other opinionated finished-content generators were removed from the v0.4.0 public surface; authored content now composes reusable helpers or uses one session-trusted script.
