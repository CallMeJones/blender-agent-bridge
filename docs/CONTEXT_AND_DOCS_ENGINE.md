...
- `create_image_texture_material(name, base_color_path, roughness_path, metallic_path, normal_path, alpha_path, emission_path, ambient_occlusion_path, arm_path, orm_path, bump_path, displacement_path, uv_map_name, object_names, selected_only)`
- `inspect_material_setup(material_names, object_names, selected_only, require_uv_maps, expected_uv_map_name, max_materials)`
- `repair_material_setup(material_names, object_names, selected_only, uv_map_name, fix_color_spaces, reconnect_uv_maps, max_materials)`
- `bake_maps(object_names, selected_only, map_types, output_dir, resolution, margin, samples, uv_map_name, overwrite)`
- `create_procedural_texture_material(name, preset, texture_type, color_a, color_b, scale, detail, distortion, bump_strength, object_names, selected_only)`
- `uv_unwrap(object_names, selected_only, method, uv_map_name, replace_existing, margin, pack_islands, projection_axis)`
- `mark_uv_seams(object_names, selected_only, mode, angle_degrees, include_boundary, clear_existing)`
- `inspect_uv_layout(object_names, selected_only, include_children, uv_map_name, max_objects, max_overlap_pairs)`
...
- `set_render_engine(engine, quality_preset, samples, denoise, view_transform, look, exposure, gamma)`
- `configure_render_outputs(enabled_passes, disabled_passes, aovs, clear_existing_aovs, pass_alpha_threshold, pass_cryptomatte_depth)`
...
- `plan_object_design(prompt, object_family, target_objects)`
...
- `create_procedural_object_kit(template, name_prefix, location, count, radius, spacing, height, style, variant, detail_level, features)`
...
- `create_lookdev_turntable_review(target_name, frame_start, frame_end, quality_preset, samples, views, resolution_x, resolution_y)`
...
`inspect_simulation_bake` is the safe simulation inspection path before an agent reaches for custom Python. It samples evaluated rigid-body/particle/simulation state across a bounded frame range, restores the original scene frame, returns object center/bounds samples, includes `get_simulation_details` cache metadata, and reports that persistent point-cache baking was not performed. This gives the client concrete bake/cache evidence without mutating persistent simulation caches or writing disk caches behind the user's back. `stage_persistent_simulation_bake` is the explicit persistent-bake path: it refuses while trust is off and runs a fixed-template scene-wide `bpy.ops.ptcache.bake_all(bake=True)` script immediately while session trust is active, without asking clients to invent bake Python. Requested object names limit inspection and cache-range preparation, but Blender's bake-all operator remains scene-wide. `analyze_center_of_mass` uses convex-hull support footprints from support objects' world-space bounds when available, and character subjects can use weighted child-mesh bounds for a better articulated center-of-mass proxy, so rotated supports and rigged body-part layouts do not get reduced to a single misleading object origin or axis-aligned box.

The advanced helpers are intentionally bounded. They create useful starter states and simple edits without exposing arbitrary node graph, rig, compositor, or persistent simulation mutation. `plan_director_workflow` is the read-only orchestration entry point for broad scene, asset, animation, evidence, and preview decision work; it composes inspection, `plan_asset_import_workflow`, `plan_advanced_scene_workflow`, animation workflow/review/repair, visual evidence, and explicit commit/revert decision options without mutating the scene or flattening those decisions into executable calls. `plan_advanced_scene_workflow` is the domain routing entry point for advanced 3D, 2D/storyboard, advanced animation, simulation setup, asset import, compositor/render, and script-heavy requests. `plan_object_design` is the read-only object-family mapper for open-ended object creation: it maps a natural-language brief to family, strategy, bounded kit arguments, material hints, generic modeling helpers, asset-reference boundaries, and script fallback policy before the client chooses creation tools. When object names are not known until a creation/import helper runs, it returns evidence checks under `deferred_tool_calls` with an `input_handoff` instead of pretending the checks can run immediately. Exact/reference asset requests suppress procedural kit arguments and may expose only a `refinement_template` hint for later post-import cleanup. `plan_asset_import_workflow` maps external asset requests to discovery, async download/cache, queued import, post-import organization/staging, and viewport evidence instead of synchronous fallback tools.

2D helpers currently cover storyboard/animatic panels and cutout layers. Material helpers include bounded shader presets such as brushed metal, brushed chrome, painted enamel, clear glass, emissive accents, rubber black, warm wood, and screen glow, local image/PBR texture wiring through `create_image_texture_material` with packed ARM/ORM channel support, AO multiply, bump maps, displacement warnings, and UV-map selection, `inspect_material_setup` / `repair_material_setup` for texture file, PBR color-space, shader-link, and UV-vector quality gates, bounded AO/normal/base-color map baking through `bake_maps`, bounded procedural texture materials through `create_procedural_texture_material` for noise/voronoi/wave/checker presets, plus `uv_unwrap`, `mark_uv_seams`, and `inspect_uv_layout` for preview-safe texture-coordinate setup, boundary/angle seam marking, layout bounds/area stats, failing possible-overlap checks, and scale warnings. Look-dev helpers include `set_render_engine` and `set_render_settings` for quality presets, samples, denoise, color management, resolution, and timeline settings, `configure_render_outputs` for ViewLayer render passes, cryptomatte/alpha settings, and shader AOVs, plus `create_lookdev_turntable_review` to compose bounded stage/turntable setup, render controls, inspection still capture, and artifact validation. Procedural/modeling helpers cover reversible Geometry Nodes starter templates (`passthrough`, `transform`, `join_geometry`, `set_position`, `subdivide_mesh`), object-kit templates including kitbash, radial/scatter/product, desk-lamp/product-prop, mechanical-joint, control-panel, studio-prop, mechanical-part, modular-wall-panel, and pipe-run generators, non-destructive modifier stacks for procedural arrays, Booleans, mirror, symmetry, solidify thickness, and screw/spiral depth, plus bounded mesh-data edits (`edit_mesh`) for extrude, inset, bounded planar loop cuts, planar knife cuts, boundary-loop bridge, dissolve-degenerate, merge-by-distance, proportional vertex edits, and UV projection on single-user mesh data with snapshot rollback. Object kits are design grammars rather than fixed assets: `create_procedural_object_kit` accepts bounded `style`, `variant`, `detail_level`, and semantic `features`, with the desk-lamp kit currently supporting architect/task/studio/compact/clamp variants, spring arms, counterweights, wide shades, labels, clamps, bulbs, cables, and inspection lighting. `inspect_modeling_quality` is the read-only Phase 1 quality gate for loose/non-manifold geometry, missing materials, modifier seed warnings, and scale warnings before repair or exit-test signoff. `curve_to_mesh` creates preview-safe mesh copies from curve/text sources without destroying the original. Asset plans with no known provider or no concrete asset identifier stop after discovery and require choosing a provider plus asset id/uid before download/import/presentation calls are emitted. Directed-shot helpers cover common camera/reveal/path/turntable/crane/truck shots. Compositor support is inspection/planning-first until node-tree rollback exists. When an external agent needs a custom shader-node graph beyond the bounded PBR/procedural helpers, high-to-low or custom bake workflow, production rig, compositor graph, broad destructive mesh operation beyond the bounded `edit_mesh` operations, custom provider import/export logic, custom Grease Pencil stroke editing, or complex simulation setup, it should inspect context/docs and use Python only while binary session trust is active.
Reusable refinement templates should stay bounded and composable. Vehicle, product, and character kits inspect bounds, add tasteful primitive/curve/material details, preserve preview rollback, and use session-trusted Python for topology-heavy work when necessary.
...
The client should send these fields with the request. The script runner validates syntax/size, records advisory findings, and requires active binary session trust before execution.
...
Long final-quality renders use `start_render_job` instead of session-trusted Python or blocking still/render helpers. The tool saves a temporary `.blend` copy, launches a background Blender process, and returns a `job_id`, rough estimate, and poll interval before the MCP request can time out. `quality=auto` keeps final renders at final-quality defaults, but playblast/preview/review/draft job names or notes default to a low-resolution preview profile unless the caller passes `quality`, `resolution_x`, `resolution_y`, or `samples`. `get_render_job_status` reports status, elapsed time, frame rate, estimated remaining time, frame counts, progress, output paths, log tails, and exact frame resource URIs under `blender://render-jobs/{job_id}/...`; `cancel_render_job` stops tracked jobs. For PNG sequences, `assemble_render_job_video` starts a background MP4 assembly pass and `validate_render_job_output` checks frame completeness, MP4 presence/size, and useful resource URIs before the client reports success. This is the intended route for 1080p/4K animation renders, frame sequences, MP4 quality checks, render previews, or high-sample playblasts. Large `render_scene_thumbnail` requests are guarded and should be rerouted to `start_render_job` unless the client explicitly sets `allow_blocking_render`.
