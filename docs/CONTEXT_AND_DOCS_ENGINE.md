...
Helpers should validate inputs and return structured results. With active trust, external agents should default authored object generation, modeling, animation, materials, nodes, rigging, and look development to one cohesive Python script unless the user requests helpers. Operational work and isolated requested edits remain helper-backed.
...
The advanced helpers are intentionally bounded. `plan_director_workflow` is the read-only orchestration entry point for broad scene, asset, animation, evidence, and preview-decision work. `plan_advanced_scene_workflow` routes advanced 3D, 2D/storyboard, animation, simulation, asset-import, and compositor/render requests without generating finished content. `plan_asset_import_workflow` maps external asset requests to discovery, asynchronous download/cache, queued import, post-import organization/staging, and viewport evidence.

The bridge does not include opinionated finished-content generators. Storyboards, cutouts, props, vehicles, products, characters, and shot designs are composed from reusable text, curve, mesh, material, staging, camera, animation, asset-import, and visual-evidence helpers. Open-ended authored content that cannot be expressed safely by those operations uses one custom script while binary session trust is active.

Material helpers cover bounded shader presets, image/PBR texture wiring, setup inspection/repair, map baking, procedural textures, UV unwrapping, seam marking, and layout inspection. Look-dev helpers cover render engines/settings, render passes/AOVs, turntable review, inspection stills, and artifact validation. Procedural/modeling helpers cover reusable Geometry Nodes starters, non-destructive array/Boolean/mirror/symmetry/solidify/screw operations, and bounded mesh-data edits through `edit_mesh`. `inspect_modeling_quality` is the read-only quality gate, and `curve_to_mesh` creates preview-safe mesh copies without destroying source curves or text. Compositor support remains inspection/planning-first until node-tree rollback exists.
...
`capture_animation_playblast` captures bounded viewport PNG samples and records resource URIs plus the actual scene frame for each image. `review_playblast_against_brief` turns those samples into coverage, framing, motion, repeated-action, and repair evidence. `run_animation_task` is the compact route for common animation prompts; `run_animation_workflow` adds planning, allowlisted bounce/turntable/reveal/pulse/progressive-bounce and camera-orbit generation, structured review, optional capture, and bounded repair. Underspecified custom moves, crane/truck paths, and designed shots return an explicit blocker requesting coordinates or a path instead of inventing content. Rig-driven repair follows an inspect-then-edit path through rig details, custom-property keyframes, and pose holds. Capture requires an interactive Blender window and fails soft in background mode.
