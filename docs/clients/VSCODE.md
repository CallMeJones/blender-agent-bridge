...
```json
["--from", "blender-bridge==0.5.3", "blender-bridge", "--bridge-url", "http://127.0.0.1:<port>"]
```

On Windows set `command` to `cmd` and prefix args with `"/c", "uvx"`. Cline and Roo Code expose MCP management in their extension UI and commonly accept the `mcpServers` JSON shape copied by Blender; use that UI rather than changing native VS Code's `servers` schema. Preserve any generated environment fields.
