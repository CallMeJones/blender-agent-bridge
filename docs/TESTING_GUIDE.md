...
- The published 0.4.0 artifacts were verified on 2026-07-23. The current 0.5.2 release candidate contains 503 unit tests and a complete Blender-background suite; tagged CI repeats the supported Blender matrix under Xvfb on Linux.
...
$BlenderTests = @(
  ...
  "tests\smoke_reference_image_intake.py",
  "tests\smoke_reference_part_graph.py",
  "tests\smoke_render_jobs.py",
  "tests\smoke_safe_editing_helpers.py",
  "tests\smoke_semantic_sculpt.py",
  "tests\smoke_shape_program.py",
...
- `Copy MCP Config` produces compact JSON and includes `env` only for real token/auth/runtime values.
...
- `blender_bridge_status` reports matching add-on, bridge, MCP server, config version, and runtime source hash.
...
- Map baking writes bounded AO, normal, and base-color/diffuse PNG artifacts from UV-mapped mesh materials and reports non-empty file metadata.
...
- Look-dev turntable review creates bounded staging/turntable setup, captures inspection stills, and validates rendered artifacts without draft scripts.
...
- `scripts\live_workflow_sweep.py` now covers the major bridge workflow families, including live procedural texture material setup, render pass/AOV setup, and a look-dev turntable review artifact, but there is still no exhaustive automated happy-path sweep that invokes every dispatcher tool through the live bridge.
