...
`draft_privileged_script` is a compatibility endpoint that always refuses privileged generated Python. Use the bounded external-asset, project-file, render, capture, save, or cache tool for the intended outcome. Those tools validate providers, paths, limits, and result metadata instead of granting arbitrary Python filesystem or network access.
