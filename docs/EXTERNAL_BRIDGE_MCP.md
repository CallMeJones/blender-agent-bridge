...
| `design a futuristic wall-mounted coffee machine with chrome pipes and a small display` | `plan_object_design` -> `create_procedural_object_kit(template=coffee_machine)` -> `edit_mesh` / `curve_to_mesh` / `uv_unwrap` / `create_shader_material` -> `inspect_modeling_quality` | The client maps the open-ended object brief to a family and helper path before any `draft_script`. |
