...
| `create a believable desk lamp product prop and capture inspection renders` | `plan_advanced_scene_workflow` -> `create_procedural_object_kit(template=desk_lamp)` -> `inspect_modeling_quality` -> `capture_object_inspection_renders` | The client uses a bounded semantic prop kit plus visual evidence instead of low-level primitive assembly or `draft_script`. |
