...
```text
uvx --from blender-bridge==0.5.4 blender-bridge
```
...
- `direct` exposes the gateways plus 24 curated direct helpers for compatibility evaluation.
...
The repository also ships two progressively disclosed client skills:

- `skills/blender-bridge` teaches the five-tool gateway, preview/trust contract, and recovery workflow.
- `skills/blender-reference-modeling` teaches visual decomposition, durable blind evidence scoring, bounded repair, and pending-preview completion without category builders or canned geometry.
...
For novel continuous forms, every MCP client can discover `compile_shape_program`, `inspect_shape_program`, `update_shape_program`, and `sample_shape_program_sdf`. The connected LLM authors a compact semantic graph of masses, cavities, hierarchy, and tapered paths; the bridge validates and decodes it into a bounded watertight uniform or adaptive-dual mesh, then persists the graph for named revisions. Adaptive mode supports automatic QEF-error refinement and explicit local sphere/box regions without allocating a dense global high-resolution grid. This path uses no external image-to-3D provider and no category-specific base-mesh catalog. See [IMPLICIT_SHAPE_PROGRAMS.md](IMPLICIT_SHAPE_PROGRAMS.md).

For reference-driven creature or character modeling, every MCP client can discover the part and fur sequence through catalog search: `create_reference_part_graph`, `build_part_aware_base_mesh`, `create_eye_stack`, `create_muzzle_stack`, `create_ear_stack`, `create_part_weight_vertex_groups`, `create_fur_flow_field_from_parts`, and `create_directional_fur_curves`. Part-derived vertex groups provide durable editable surface masks; the fur-flow helper attaches those masks to named density and tangent-flow regions and can apply them through the existing preview-safe groom helper. See [REFERENCE_PART_MODELING.md](REFERENCE_PART_MODELING.md).
...
For substantial authored work, `start_execution_trace` creates a compact cross-process trace. Gateway and Blender calls record redacted arguments, compact results, durations, contracts, and generated Python as local artifacts. The initiating prompt is hashed and counted rather than stored verbatim. `finalize_execution_trace` stores the observed outcome and provider-reported token usage. `prepare_execution_trace_replay` returns a dry-run plan; it never replays mutations automatically and withholds stored code unless explicitly requested.
...
For broad advanced work, search for and invoke `plan_advanced_scene_workflow` or `get_2d_animation_details` through the gateway when domain boundaries or scene context are unclear. Planning and inspection still precede mutation. The advanced and director planners return gateway-ready `next_tool_calls`; under active trust, authored domains include one deferred `draft_script` handoff with a completion gate. Resolve its inputs and stop on missing briefs, unresolved asset selection, or blocked scene context before replacing the code placeholder. Open-ended authored content does not need to prove a helper gap. The bridge deliberately does not include finished-content generators for props, vehicles, products, characters, storyboards, cutouts, or designed shots. Bounded helpers remain the control plane for assets, project files, long jobs, evidence, and preview decisions.
...
For advanced animation, use `plan_animation_workflow` to create the brief, animation-aware scene context, timing chart, and hardening checks. With active trust, authored generation then defaults to one cohesive `draft_script`; evaluator, playblast, evidence, and repair-review helpers follow it. `run_animation_task` and `run_animation_workflow` remain the bounded helper alternative when trust is off, the user requests helpers, or one exact reusable motion fully expresses the brief. `get_animation_scene_context` remains the lower-level routing tool for likely edit targets, rig-driven meshes, rig control candidates, shape keys, drivers, constraints, physics hints, contact surfaces, camera readiness, subject routing, recommended deeper inspection tools, and an `animation_hardening` block listing risk flags, required pre-mutation inspections, review tools, and repair-loop limits.
...
| --- | --- | --- |
| `make selected cube bounce twice and get smaller each bounce` | `plan_animation_workflow` -> cohesive `draft_script` -> review tools | With trust active, the client plans timing and then uses one authored script; helper generation is the trust-off or explicit-helper path. |
| `block a jump with anticipation, contact, apex, settle` | `plan_animation_workflow` -> cohesive `draft_script` -> evaluators | The client establishes brief, targets, and timing before one authored animation script. |
...
| `create a 2D storyboard animatic with panels and a camera move` | `plan_advanced_scene_workflow` -> `get_2d_animation_details` -> cohesive `draft_script` -> evidence helpers | Authored panel, vector, and camera construction uses one trusted script after inspection. |
| `make an advanced procedural hard-surface array stack` | `plan_advanced_scene_workflow` -> `get_geometry_nodes_details` -> cohesive `draft_script` | Under trust, coordinated procedural construction is script-first; `apply_procedural_array_stack` remains the explicit helper alternative. |
| `use a boolean cutter, mirror the model, symmetrize it, solidify wall thickness, and add a screw thread` | `plan_advanced_scene_workflow` -> cohesive `draft_script` -> `inspect_modeling_quality` | One script coordinates the full modifier/modeling pass instead of fragmenting it across calls. |
| `extrude mesh faces, inset panels, loop cut, knife cut, proportional edit, bridge boundary loops, merge by distance, and convert curve to mesh` | `plan_advanced_scene_workflow` -> cohesive `draft_script` -> inspection | One checkpoint-backed script owns the coordinated mesh edit; exact helpers remain available on request. |
...
| `mark hard-edge UV seams, pack UV islands, inspect for overlaps, apply a packed ARM PBR material, and render a small lookdev still` | `plan_advanced_scene_workflow` -> cohesive `draft_script` for the coordinated UV/material pass -> `inspect_uv_layout` -> `create_lookdev_turntable_review` | Authored UV/material coordination is script-first under trust; bounded UV and material helpers remain the explicit-helper alternative, while lookdev evidence stays helper-backed. |
...
| `create a procedural marble texture material with noise bump for the selected mesh` | cohesive `draft_script` -> `inspect_material_setup` | Authored material/node construction is script-first under trust; the bounded preset remains available when requested. |
| `design a futuristic wall-mounted coffee machine with chrome pipes and a small display` | `plan_advanced_scene_workflow` -> cohesive `draft_script` -> `inspect_modeling_quality` | No product-specific generator is exposed; one brief-derived script owns the authored build. |
| `create a believable architect desk lamp with spring arms and capture inspection renders` | `plan_advanced_scene_workflow` -> cohesive `draft_script` -> `capture_object_inspection_renders` | No prop grammar is exposed; the authored build remains script-first and evidence remains helper-backed. |
| `create an advanced camera push and orbit reveal` | `plan_animation_workflow` -> cohesive `draft_script` -> visual review | The script coordinates the authored camera move after target and timing inspection. |
...
If a trusted client still fragments authored generation across helpers instead of selecting `draft_script`, refresh or restart the MCP client, press `Copy MCP Config` again, confirm `tools/list` contains the five gateway tools, search the catalog for the authored goal, and check `blender_bridge_status` for matching versions, active script trust, and `mcp_tool_surface: gateway`.
...
- Trust can be enabled before or after starting the bridge. It is cleared by **Revoke**, timed expiry, add-on reload, or Blender exit. Starting or stopping the bridge and opening, creating, restoring, copying, renaming, saving, or modifying `.blend` files do not change an active grant. File operations never extend a timed grant's expiry. Static findings remain advisory after trust.
