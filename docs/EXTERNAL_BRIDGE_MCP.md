...
| `unwrap the cube, apply a packed ARM PBR material, enable Cycles denoise, and render a small lookdev still` | `uv_unwrap` -> `create_image_texture_material(arm_path=..., uv_map_name=...)` -> `create_lookdev_turntable_review` | The client uses bounded Phase 2 look-dev helpers, inspection stills, and artifact validation before custom shader/render Python. |
