...
`draft_privileged_script` is a compatibility endpoint that always refuses privileged generated Python. Use the bounded external-asset, project-file, render, capture, save, or cache tool for the intended outcome. `list_project_files`, `read_project_file`, and `write_project_file` expose the current saved `.blend` directory only; they reject unsaved projects, traversal, absolute/hidden paths, and links, and cap generic reads/writes at 4 MiB. Generic writes also reject executable/script/library and `.blend` targets. These tools validate providers, paths, limits, and result metadata instead of granting arbitrary Python filesystem or network access.

For broad advanced work, compact MCP mode exposes `plan_advanced_scene_workflow`, `plan_object_design`, and `get_2d_animation_details` directly. Use the planner before custom scripts when a request mixes advanced 3D, 2D/storyboard, animation, simulation, compositor/render, or unclear helper coverage. For open-ended object creation, call `plan_object_design` before choosing kits, generic modeling helpers, asset import, or scripts; it maps the prompt to object family, strategy, kit arguments, material hints, helper tools, asset-reference boundaries, and fallback policy. If the created/imported object names are not known yet, evidence calls are returned as `deferred_tool_calls` with an `input_handoff` that tells the client which prior helper result should populate `object_names`. Exact/reference asset requests clear procedural `kit_arguments` and use asset import first; any `refinement_template` is a later cleanup hint, not permission to procedurally guess the referenced object. For storyboard or animatic prompts, route through `get_2d_animation_details`, `create_storyboard_panels`, `create_2d_cutout_layer`, and optionally `create_camera_dolly_animation` or `create_directed_animation_shot`. For procedural 3D stack, kit, or basic modeling prompts, use `create_procedural_object_kit`, `apply_procedural_array_stack`, `edit_mesh`, `inspect_modeling_quality`, `curve_to_mesh`, `boolean_op`, `mirror_model`, `symmetrize_model`, `solidify_model`, `screw_model`, and geometry-node inspection before custom mesh or node scripts. Object kits are bounded design grammars: when a prompt asks for a styled or variant prop, pass `style`, `variant`, `detail_level`, and semantic `features` into `create_procedural_object_kit` before considering custom assembly scripts. The kit grammars currently include desk lamps, coffee machines, control panels, mechanical parts/assemblies, modular wall panels, pipe runs, product display rigs, product stacks, radial arrays, scatter grids, and kitbash towers. The desk-lamp grammar supports architect/task/studio/compact/clamp variants and features such as spring arms, counterweight, wide shade, clamp, labels, bulb, cable, and inspection lights. `edit_mesh` is intentionally bounded to single-operation topology/shape edits on single-user mesh data with live-preview mesh snapshots, including extrude, inset, bounded planar loop cut, planar knife cut, bridge, dissolve, merge, and proportional edits. `inspect_modeling_quality` is read-only and should be used for loose/non-manifold geometry, missing-material, modifier-seed, and unapplied-scale checks before scripted repair. For cloth setup, use `add_cloth_simulation_to_selected`, then inspect simulation state before any persistent bake.
...
Use these prompts after installing a fresh zip and refreshing the MCP client. During routing tests, revoke script trust to verify that `draft_script` is refused without pending state, then enable trust to verify immediate execution after static checks. Custom asset/project-file work should route to bounded structured tools, and persistent-bake scripts remain disabled.
...
| `design a futuristic wall-mounted coffee machine with chrome pipes and a small display` | `plan_object_design` -> `create_procedural_object_kit(template=coffee_machine)` -> `edit_mesh` / `curve_to_mesh` / `uv_unwrap` / `create_shader_material` / `create_image_texture_material` -> `inspect_modeling_quality` | The client maps the open-ended object brief to a family and helper path before any `draft_script`. |
...
- Generated Python uses one binary, runtime-only switch. Trust off refuses `draft_script` without staging; trust on immediately runs ordinary scripts after static checks.
- `run_approved_script` and `draft_privileged_script` remain compatibility endpoints but always refuse execution. There are no approval tokens or pending-script actions.
- Trust can be enabled before or after starting the bridge. It is cleared by **Revoke**, add-on reload, file load, or Blender exit. Privileged and persistent-bake scripts remain disabled.
