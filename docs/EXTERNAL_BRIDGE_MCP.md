...
For novel continuous forms, every MCP client can discover `compile_shape_program`, `inspect_shape_program`, `update_shape_program`, and `sample_shape_program_sdf`. The connected LLM authors a compact semantic graph of masses, cavities, hierarchy, and tapered paths; the bridge validates and decodes it into a bounded watertight mesh, then persists the graph for named revisions. This path uses no external image-to-3D provider and no category-specific base-mesh catalog. See [IMPLICIT_SHAPE_PROGRAMS.md](IMPLICIT_SHAPE_PROGRAMS.md).
