...
| `unwrap the cube, apply a packed ARM PBR material, enable Cycles denoise, and render a small lookdev still` | `uv_unwrap` -> `create_image_texture_material(arm_path=..., uv_map_name=...)` -> `set_render_engine` / `set_render_settings` -> `capture_object_inspection_renders` | The client uses bounded Phase 2 look-dev helpers before custom shader/render Python. |
