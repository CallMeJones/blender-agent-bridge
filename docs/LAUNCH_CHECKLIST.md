...
The completed dashboard and evidence below are historical proof for immutable published artifacts. Current `main` is 0.5.4 development and is **not** covered by the v0.5.3 release evidence.

- [ ] **Required** — Run the complete unit, pure-Python, supported-Blender, packaging, and clean installed-extension gates against the final 0.5.4 candidate commit.
...
- [ ] **Required** — Move every Unreleased changelog item into the 0.5.4 section, advance `release_state.toml`, update every tracked public install pin, and rerun release consistency before creating `v0.5.4`. The workflow generates and identity-checks `public/index.json` from the candidate artifact.
- [ ] **Manual** — Do not tag, publish, deploy, or announce 0.5.4 until the required gates above are complete and explicitly approved.
