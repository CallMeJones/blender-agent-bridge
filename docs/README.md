...
Some MCP clients cache tool lists and server configs. After installing a new zip or pressing `Copy MCP Config`, replace the old client config and refresh or restart the MCP client. The copied config is compact by default; it includes an `env` block only when a bridge token, Sketchfab token, or optional `uvx / PyPI` runtime marker is needed. Use `blender_bridge_status` for version, source, and tool-registry diagnostics.
