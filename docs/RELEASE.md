...
- `resources/list` includes capture, playblast, inspection-render, and render-thumbnail resources, and `resources/read` can read `blender://captures/latest/metadata` after a capture, `blender://playblasts/latest/metadata` after a playblast capture, `blender://inspection-renders/latest/metadata` after diagnostic object renders, plus `blender://render-thumbnails/latest/metadata` after thumbnail renders.
