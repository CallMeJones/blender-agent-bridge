...
If a tag run publishes PyPI but fails before Pages or the GitHub Release completes, do not move the tag and do not rebuild publication files. Run the manual **Resume tagged release** workflow with the immutable tag and the original tag workflow run id. The recovery workflow binds that run to the tag commit, reuses its retained tested artifacts, requires the wheel and sdist hashes to match the complete PyPI release, verifies extension/Pages archive identity, uploads a fresh Pages artifact, publishes the GitHub Release from the same retained files, and repeats public artifact-identity verification. A newly rebuilt wheel or sdist is intentionally rejected because Python archives are not guaranteed to be byte-reproducible across separate builds.
