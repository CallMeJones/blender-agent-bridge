...
If a tag run publishes PyPI but fails before Pages or the GitHub Release completes, do not move the tag and do not rebuild publication files. Run the manual **Resume tagged release** workflow with the immutable tag, a tag workflow run containing a tested extension/Pages artifact set, and the tag workflow run containing the exact wheel/sdist already published to PyPI. Both retained runs must resolve to the tag commit. The recovery workflow requires the wheel and sdist hashes to match the complete PyPI release, verifies extension/Pages archive identity, uploads a fresh Pages artifact, publishes the GitHub Release from those retained files, and repeats public artifact-identity verification. A newly rebuilt wheel or sdist is intentionally rejected because Python archives are not guaranteed to be byte-reproducible across separate builds.
...
- External script trust can be turned on/off, persists across bridge stop/start and every `.blend` lifecycle operation, and clears only on explicit Revoke, timed expiry, add-on reload, or Blender exit. File open, new, save, restore, copy, rename, and content changes do not alter the grant or extend timed expiry.
