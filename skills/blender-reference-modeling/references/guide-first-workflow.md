...
- Prefer `compile_shape_program` when the subject can be expressed as named continuous masses, smooth boolean cavities, and tapered paths. Store `semantic_role` labels, use `parent_id` for attached forms, inspect the canonical graph, and revise exact nodes with `update_shape_program` after each measured comparison. Use uniform mode while proportions move, then adaptive-dual mode with local sphere/box refinement regions around measured detail; verify the returned per-region surface counts.
