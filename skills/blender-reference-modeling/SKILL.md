...
Use `compile_shape_program` for primary continuous construction when the reference can be decomposed into semantic primitives and tapered sweeps. Start at low resolution, leave margin in explicit object-local program bounds, use `sample_shape_program_sdf` when a boolean or cavity is uncertain, and keep the result in preview. Before refinement, call `inspect_shape_program(include_program=true)` and revise exact named nodes rather than replacing the mesh blindly. Use `create_reference_blockout` only when direct guide-ellipse conversion is the better scaffold.
